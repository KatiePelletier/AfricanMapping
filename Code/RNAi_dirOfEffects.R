...
ef6a_corrected.m_2 <- ef6a.m - w_loco.m
...
ef6a_corrected2.f <- ef6a.f - w_loco.f
...
cor(males.diff, t(ef6a_corrected.m_2)) #-0.4009203 changedto: -0.004480492 with the new correction
...
cor(females.diff, t(ef6a_corrected2.f)) #-0.8263311 # with new correction 0.216442
...
cor(t(ef6a_corrected.m_2), t(ef6a_corrected2.f)) #0.6519004 #after correction 0.9434931
...
m.eff.vecs <- list(males.diff, loco_corrected.m, wge_corrected.m, btn_corrected.m, ef6a_corrected.m_2, takl2_corrected.m, wake_corrected.m)
...
png("../Figures/ef6aMaleEff_2x.png")
plotRefToTarget(m, m.eff.mats[,,5], 
                method = "points",
                links = wing15.links, mag = 2, 
                gridPars=wing.spec)
dev.off()
...
#names. this was for troubleshooting but now I think this is actually worth keeping because its the order of effect mats in the array. 
f.effs <- c("loco_corrected.f", "wge_corrected.f", "btn_corrected.f", "ef6a_corrected2.f", "takl2_corrected.f", "wake_corrected.f")
...
ef6a.raw <- image_read("../Figures/ef6aMaleEff_2x.png")
...
#######
#I want to do this with just one at a time. 
rnai.males.ef6a <- droplevels(rnai.males[rnai.males$cross == "nb_ef6a" | rnai.males$cross == "w_loco", ])
levels(rnai.males.ef6a$cross)
rnai.males.ef6a$cross <- relevel(rnai.males.ef6a$cross, ref = "w_loco")
levels(rnai.males.ef6a$cross)
...
wings.pan <- plot_grid(alt.pan, loco, ef6a, wge, ncol = 1, labels = c("", "loco", "ef6a", "wge"), label_fontface = "bold.italic", hjust = -2)
